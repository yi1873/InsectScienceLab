...
ggsave(filename=paste(outdir,'/cor_pearson.pdf',sep=''),plot=p, height=max(8,round(nrow(sam_num)/3)), width=max(8,round(nrow(sam_num)/3)))
ggsave(filename=paste(outdir,'/cor_pearson.png',sep=''),type="cairo-png", plot=p, height=max(8,round(nrow(sam_num)/3)), width=max(8,round(nrow(sam_num)/3)))
